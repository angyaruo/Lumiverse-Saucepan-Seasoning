...
export function setup(ctx) {
  ...
  const TOOLBAR_H = 34;
  let baseSafeZone = 185; // Lumiverse default — overwritten on first mount
  ...
  function updateSafeZone() {
    ...
    const panelH  = panelEl?.classList.contains('ri-open') ? (panelEl.scrollHeight ?? 0) : 0;
    col.style.setProperty('--lcs-input-safe-zone', `${baseSafeZone + TOOLBAR_H + panelH}px`);
  }
  ...
  function mount() {
    ...
    // Read base value BEFORE we modify it
    const raw = getComputedStyle(parent).getPropertyValue('--lcs-input-safe-zone').trim();
    baseSafeZone = parseInt(raw) || 185;
    ...
    // Update safe zone whenever panel opens/closes
  ...
  return () => {
    ...
    const col = getColumnInner();
    if (col) col.style.setProperty('--lcs-input-safe-zone', `${baseSafeZone}px`);
  };
}
